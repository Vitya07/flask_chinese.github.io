...
def index():
    if 'user_id' in session:
        ...
        return render_template('index.html', known_chars=known_chars)
...
def add_char():
    if 'user_id' in session:
        ...
        if char and len(char) == 1:
            ...
            if existing_char:
                flash(f'Иероглиф {char} уже добавлен.')
            else:
                conn.execute('INSERT INTO known_chars (user_id, char) VALUES (?, ?)', (user_id, char))
                conn.commit()
                flash(f'Иероглиф {char} добавлен.')
            conn.close()
        else:
            flash('Пожалуйста, введите один иероглиф.')
    return redirect(url_for('index'))

@app.route('/remove_char', methods=['POST'])
def remove_char():
    if 'user_id' in session:
        user_id = session['user_id']
        char = request.form['char']
        ...
        conn.commit()
        conn.close()
        flash(f'Иероглиф {char} удален.')
    return redirect(url_for('index'))

@app.route('/find_words')
def find_words_route():
    if 'user_id' in session:
        user_id = session['user_id']
        conn = get_db_connection()
        known_chars = conn.execute('SELECT char FROM known_chars WHERE user_id = ?', (user_id,)).fetchall()
        ...
        translations = {word: translate_word(word) for word in words} if words else {}
        pinyins = {word: get_pinyin(word) for word in words} if words else {}
        return render_template('find_words.html', translations=translations, pinyins=pinyins)
    return redirect(url_for('index'))
